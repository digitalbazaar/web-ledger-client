...
class WebLedgerClient {
  ...
  async getDocument({service}) {
    ...
    if(response.problem) {
      console.log('PPPPPPPP', response.problem); // CLIENT_ERROR
      console.log('777777777777', JSON.stringify(response.data, null, 2));
  ...
  async getAgent({id} = {}) {
    ...
    console.log('ledgerAgent', ledgerAgent);
    // this.ledgerAgentApi = this._createLedgerAgentApi({ledgerAgent});
    return ledgerAgent;
  }
  ...
  _createLedgerAgentApi(ledgerAgent) {
    const agentId = ledgerAgent.id.split(':')[3];
    console.log('PPPPPPP', agentId);
    return create({
      baseURL: `https://${this.hostname}/ledger-agents/${agentId}`,
      httpsAgent: this.httpsAgent,
      headers: DEFAULT_HEADERS,
    });
  }
