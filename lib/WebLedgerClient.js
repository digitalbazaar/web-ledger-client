...
class WebLedgerClient {
  /**
   * @param {string} hostname {string} Hostname of the ledger node.

   * @param {('dev'|'test'|'live')} [mode=test] {string} - Determines https
   *   agent settings (does not reject unsigned certs in 'dev' mode, for
   *   example).
   * @param [logger]
   * @param {Agent} [httpsAgent]
   */
  ...
  async wrap({addCreator = true, record, operationType = 'create'}) {
    ...
    if(addCreator && this.ledgerAgent.targetNode) {
      operation.creator = this.ledgerAgent.targetNode;
    }
